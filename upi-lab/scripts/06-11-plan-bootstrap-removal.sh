...
[[ -s "$CLUSTER_STAGE_FILE" && "$(<"$CLUSTER_STAGE_FILE")" == steady ]] \
  || phase6_error 'HAProxy/DNS steady-state marker is missing. Run scripts/06-10-cutover-from-bootstrap.sh first.'
...
printf 'After the review passes: bash scripts/06-12-apply-bootstrap-removal.sh\n'
