...
section "Local tools"
for command_name in aws terraform ansible oc openshift-install jq git openssl ssh-keygen ssh-agent ssh-add flock; do
...
if registered_account_id="$(lab_resolve_expected_account_id strict "$TERRAFORM_DIR" '')"; then
  ...
  fail "Run scripts/02-01-register-expected-account.sh and register the approved 12-digit AWS Account ID before continuing"
