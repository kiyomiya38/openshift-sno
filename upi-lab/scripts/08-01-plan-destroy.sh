...
operation='plan'
if (($# == 1)) && [[ "$1" == '--discard-saved-plan' ]]; then
  operation='discard-saved-plan'
elif (($# != 0)); then
  printf 'Usage: %s [--discard-saved-plan]\n' "${0##*/}" >&2
  exit 2
fi
...
if [[ "$operation" == 'discard-saved-plan' ]]; then
  if [[ ! -e "$PLAN_FILE" && ! -e "$PLAN_META_FILE" ]]; then
    printf 'No saved destroy Plan or guard metadata exists.\n'
    exit 0
  fi

  printf 'This removes only the following local saved Plan files; it does not change AWS or Terraform state:\n'
  [[ ! -e "$PLAN_FILE" ]] || printf '  %s\n' "$PLAN_FILE"
  [[ ! -e "$PLAN_META_FILE" ]] || printf '  %s\n' "$PLAN_META_FILE"
  read -r -p 'Type DISCARD-SAVED-DESTROY-PLAN to continue: ' CONFIRM
  if [[ "$CONFIRM" == 'DISCARD-SAVED-DESTROY-PLAN' ]]; then
    rm -f -- "$PLAN_FILE" "$PLAN_META_FILE"
    printf 'Saved destroy Plan and guard metadata were discarded. Terraform state and AWS were unchanged.\n'
  else
    printf 'Destroy Plan discard cancelled.\n'
  fi
  exit 0
fi
...
managed_state="$(printf '%s' "$state_list" | sed '/^data\./d; /^$/d')"
[[ -n "$managed_state" ]] || {
  printf 'No managed Terraform resources remain. No destroy Plan was created.\n'
  printf 'Next: bash scripts/08-03-delete-client-vpn-certificate.sh\n'
  exit 0
}
managed_count="$(printf '%s' "$managed_state" | grep -c '^')"
...
printf 'Next: review the saved destroy Plan, then disconnect the Windows Client VPN.\n'
printf '  terraform -chdir=%q show -no-color %q\n' "$TERRAFORM_DIR" "$PLAN_FILE"
printf 'After the review and VPN disconnect: bash scripts/08-02-apply-destroy.sh\n'
