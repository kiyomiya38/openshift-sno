...
[[ "$managed_count" == 33 ]] || {
  printf 'ERROR: Clean infrastructure planning requires exactly 33 managed Network and Client VPN resources; found %s.\n' "$managed_count" >&2
  printf 'Use the partial-state cleanup procedure instead of planning across construction stages.\n' >&2
  exit 1
}

if grep -q '^aws_vpc_dhcp_options\.cluster\[' <<<"$state_list"; then
  printf 'ERROR: OpenShift cluster prerequisites already exist. This infrastructure planner would omit active OpenShift construction flags.\n' >&2
  printf 'Use the dedicated OpenShift installation scripts or scripts/08-01-plan-destroy.sh.\n' >&2
  exit 1
fi
...
printf 'After the review passes: bash scripts/05-02-apply-infrastructure.sh\n'
