...
actual_managed_state="$(terraform -chdir="$TERRAFORM_DIR" state list 2>/dev/null |
  grep -v '^data\.' | LC_ALL=C sort || true)"
[[ "$actual_managed_state" == "$(LC_ALL=C sort <<<"$expected_network_state")" ]] || {
  printf 'ERROR: Client VPN planning requires exactly the validated 26-resource Network state and no later-phase resources.\n' >&2
  exit 1
}
...
printf 'Saved plan: %s\n' "$PLAN_FILE"
printf 'Next: bash scripts/03b-apply-client-vpn.sh\n'
