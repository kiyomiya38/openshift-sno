...
terraform -chdir="$TERRAFORM_DIR" state show 'aws_vpc_dhcp_options.cluster[0]' >/dev/null 2>&1 \
  || phase6_error 'Cluster prerequisites are not in Terraform state. Complete scripts/16 and scripts/17 first.'
...
[[ "$ignition_sg_id" == sg-* ]] \
  || phase6_error 'Ignition Server Security Group output is missing. Complete both scripts/16 and scripts/17 passes first.'
jq -e --arg security_group_id "$ignition_sg_id" \
  'index($security_group_id) != null' <<<"$installer_sg_ids_json" >/dev/null \
  || phase6_error 'Installer does not have the Ignition Server Security Group. Run scripts/16 and scripts/17 again before generating Ignition.'
...
if [[ -s "$INSTALL_DIR/install-config.yaml" ]]; then
  ...
  phase6_error 'Neither install-config.yaml nor a complete generated Ignition set exists. Run scripts/15 again.'
fi
...
printf 'Next: bash scripts/19-validate-cluster-prerequisites.sh\n'
