...
[[ ! -e "$HAPROXY_RECOVERY_FILE" ]] \
  || error "An unfinished HAProxy recovery exists. Run scripts/07-04-test-haproxy-failover.sh --recover first."
...
wait_for_platform_stable() {
  ...
  for ((attempt = 1; attempt <= 180; attempt++)); do
    ...
    if [[ "$pending_count" != 0 ]]; then
      ...
      printf 'Review it with: bash scripts/06-13-review-csrs.sh\n' >&2
      return 1
    fi
...
print_recovery_notice() {
  ...
  printf '  bash scripts/07-05-test-worker-reboot.sh --recover\n' >&2
}
...
assert_no_pending_csrs \
  || error 'A pending CSR exists. Review it with scripts/06-13-review-csrs.sh.'
wait_for_workloads_ready \
...
if "$PREFLIGHT_ONLY"; then
  ...
  printf 'Next: bash scripts/07-05-test-worker-reboot.sh %s\n' "$TARGET_NAME"
  exit 0
fi
...
assert_no_pending_csrs \
  || error 'A pending CSR exists. Do not auto-approve it; run scripts/06-13-review-csrs.sh.'
wait_for_workloads_ready "$NODE_NAME" \
...
case "$TARGET_NAME" in
  worker-0)
    printf 'Next after a fresh preflight: bash scripts/07-05-test-worker-reboot.sh --preflight-only worker-2\n'
    ;;
  worker-2)
    printf 'Next after a fresh preflight: bash scripts/07-05-test-worker-reboot.sh --preflight-only worker-1\n'
    ;;
  worker-1)
