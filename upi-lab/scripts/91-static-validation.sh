...
printf '=== Bash ===\n'
mapfile -d '' shell_files < <(find "$SCRIPT_DIR" "$LAB_ROOT/tests" -type f -name '*.sh' -print0 | sort -z)
...
if bash "$LAB_ROOT/tests/test-empty-terraform-state.sh"; then
  pass 'Empty Terraform state is handled safely.'
else
  fail 'Empty Terraform state safety regression failed.'
fi

if bash "$LAB_ROOT/tests/test-phase6-node-inventory.sh"; then
  pass 'Phase 6 exact node inventory safety checks passed.'
else
  fail 'Phase 6 exact node inventory safety regression failed.'
fi

if bash "$LAB_ROOT/tests/test-phase6-csr-eligibility.sh"; then
  pass 'Phase 6 CSR eligibility safety checks passed.'
else
  fail 'Phase 6 CSR eligibility safety regression failed.'
fi

if bash "$LAB_ROOT/tests/test-local-cleanup-archive-maintenance.sh"; then
  pass 'Local cleanup archive-maintenance safety checks passed.'
else
  fail 'Local cleanup archive-maintenance safety regression failed.'
fi
...
if command -v python3 >/dev/null 2>&1; then
  ...
  if python3 "$SCRIPT_DIR/90-validate-runbook-order.py"; then
    pass 'Numbered content, review, Apply, and validation order is consistent.'
  else
    fail 'Runbook execution order validation failed.'
  fi
  ...
  if python3 -c 'import yaml' >/dev/null 2>&1; then
    ...
    if python3 "$SCRIPT_DIR/90-validate-release-pins.py"; then
      pass 'OpenShift release versions, archive names, and checksums are consistent.'
    else
      fail 'OpenShift release pin validation failed.'
    fi
  else
    skip 'PyYAML is not installed; YAML and release pin validation were skipped.'
  fi
else
