...
[[ ! -e "$NODE_REBOOT_RECOVERY_FILE" ]] \
  || error "An unfinished worker reboot recovery exists. Run scripts/34-test-worker-reboot.sh --recover first."
...
print_recovery_notice() {
  ...
  printf '  bash scripts/33-test-haproxy-failover.sh --recover\n' >&2
}
...
if "$PREFLIGHT_ONLY"; then
  ...
  printf 'Next: bash scripts/33-test-haproxy-failover.sh %s\n' "$TARGET_NAME"
  exit 0
fi
...
if [[ "$TARGET_NAME" == haproxy-0 ]]; then
  printf 'Optional next test after a fresh preflight: bash scripts/33-test-haproxy-failover.sh --preflight-only haproxy-1\n'
fi
