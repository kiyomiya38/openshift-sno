...
managed_count="$(lab_managed_state_count_allow_absent "$TERRAFORM_DIR")" || exit 1
...
if [[ -n ${EXPECTED_CLIENT_VPN_DNS_SERVERS:-} ]]; then
  printf 'Next: complete the Windows DNS checks in docs/05-infrastructure-services.md section 13.5.\n'
else
  printf 'Next: bash scripts/04-06-export-client-vpn-config.sh\n'
fi
