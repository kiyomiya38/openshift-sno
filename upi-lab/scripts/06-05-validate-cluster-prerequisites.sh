...
state_list="$(lab_state_list_required "$TERRAFORM_DIR")" \
  || phase6_error 'Cluster prerequisite validation requires readable Terraform state.'
node_count="$(printf '%s' "$state_list" | grep -c '^aws_instance\.openshift\[' || true)"
...
printf 'Next: bash scripts/06-06-plan-cluster-nodes.sh\n'
